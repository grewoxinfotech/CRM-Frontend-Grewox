...
import { FiX, FiDollarSign, FiFileText, FiUser, FiEdit2 } from "react-icons/fi";
...
const EditProposal = ({ open, onCancel, initialValues, onSuccess }) => {
  ...
  useEffect(() => {
    if (open && initialValues) {
      // Set form values
      form.setFieldsValue({
        lead_title: initialValues.lead_title,
        date: dayjs(initialValues.valid_till),
      ...
      if (typeof initialValues.items === 'string') {
        try {
          ...
          if (Array.isArray(parsedItems)) {
            formattedItems = parsedItems.map((item, index) => ({
              id: index + 1,
              item: item.item || "",
      ...
      } else if (Array.isArray(initialValues.items)) {
        formattedItems = initialValues.items.map((item, index) => ({
          id: index + 1,
          item: item.item || "",
          quantity: item.quantity || 1,
          unit_price: item.price || 0,
      ...
      if (initialValues.discount_type && initialValues.discount_value !== undefined) {
        // Use the discount type and value directly from initialValues
        setDiscountType(initialValues.discount_type);
        setDiscountValue(parseFloat(initialValues.discount_value));
      } else if (initialValues.discount !== undefined) {
        ...
        if (subTotal > 0) {
          ...
          // Default to fixed discount if we can't calculate percentage
          setDiscountType("fixed");
          setDiscountValue(parseFloat(initialValues.discount) || 0);
        }
      } else {
        // Default values if no discount information is available
        setDiscountType("percentage");
        setDiscountValue(0);
      }
      ...
      setTotals({
        subTotal: parseFloat(initialValues.subtotal) || 0,
        discount: parseFloat(initialValues.discount) || 0,
        totalTax: parseFloat(initialValues.tax) || 0,
        totalAmount: parseFloat(initialValues.total) || 0
      });
    }
  }, [open, initialValues, form, currencies]);
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      message.error(error.data?.message || 'Failed to update proposal. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        >
          ...
          <div>
            ...
            >
              Update the information for this proposal
            </Text>
          </div>
        </div>
      </div>
              ...
              {items.map((item, index) => (
                <React.Fragment key={item.id}>
                  <tr className="item-data-row">
                    ...
                    <td>
                      <div className="amount-field" style={{
                        padding: '0 16px',
                        marginBottom: '5px',
                      }}>
