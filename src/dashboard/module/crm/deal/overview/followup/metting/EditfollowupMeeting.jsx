...
const EditFollowupMeeting = ({ open, onCancel, onSubmit, meetingId }) => {
    ...
    const { data: meetingData, isLoading: isMeetingLoading } = useGetFollowupMeetingByIdQuery(meetingId);
    ...
    useEffect(() => {
        if (meetingData?.data) {
            const meeting = meetingData.data.find(t => t.id === meetingId);

            try {
                // Parse assigned_to (participants)
                let assignedTo = meeting.assigned_to;
                ...
                // Parse reminder data
                let reminderData = meeting.reminder;
