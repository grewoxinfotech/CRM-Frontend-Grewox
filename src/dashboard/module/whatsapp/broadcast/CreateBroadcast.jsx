    ...
    import { Steps, Form, Input, Select, Button, DatePicker, Checkbox, Space, Divider, Typography, Alert, Row, Col, Card, Tag, message, Tooltip, Upload } from 'antd';
    import { 
        PlusOutlined, 
        SendOutlined, 
        ThunderboltOutlined, 
...
import * as XLSX from "xlsx";
    ...
    const CreateBroadcast = ({ onSuccess }) => {
        ...
        const totalRecipients = 
            ...
            targetModule === 'companies' ? (companyCountData?.pagination?.total || 0) : 
            targetModule === 'excel' ? (audienceFilter?.custom_numbers?.length || 0) : 0;
        ...
        const handleSubmit = async () => {
            try {
                ...
                const templateText = selectedTemplate?.components?.find(c => c.type?.toUpperCase() === 'BODY')?.text || '';
                    ...
                    <div style={{ marginTop: '20px' }}>
                        ...
                        <Form.Item name="target_module" label="Target Module" initialValue="leads">
                            <Select onChange={() => form.setFieldsValue({ target_module: form.getFieldValue('target_module') })}>
                                ...
                                <Option value="excel">Upload Excel/CSV</Option>
                            </Select>
                        </Form.Item>
                        ...
                        <Form.Item noStyle shouldUpdate={(prev, curr) => prev.target_module !== curr.target_module}>
                            {({ getFieldValue }) => 
                                ...
                                ) : getFieldValue('target_module') === 'excel' ? (
                                    <div style={{ marginBottom: '24px' }}>
                                        <Alert 
                                            message="Upload Instructions"
                                            description={
                                                <div>
                                                    <p style={{ marginBottom: '8px' }}>Please upload an Excel or CSV file. The system will look for a column named 'Phone', 'Mobile', 'WhatsApp', or 'Number' to extract the phone numbers. Ensure they have country codes (e.g., 919876543210).</p>
                                                    <Button 
                                                        type="link" 
                                                        size="small" 
                                                        ...
                                                        onClick={() => {
                                                            const sampleData = [
                                                                { Name: "John Doe", Phone: "919876543210", City: "Mumbai", Note: "New Customer" },
                                                                { Name: "Jane Smith", Phone: "919876543211", City: "Delhi", Note: "Returning Customer" }
                                                            ];
                                                            const ws = XLSX.utils.json_to_sheet(sampleData);
                                                            const wb = XLSX.utils.book_new();
                                                            XLSX.utils.book_append_sheet(wb, ws, "Sample");
                                                            XLSX.writeFile(wb, "Sample_Broadcast_Contacts.xlsx");
                                                        }}
                                                    >
                                                    ...
                                                    try {
                                                        ...
                                                        json.forEach(row => {
                                                            ...
                                                            if (phoneKey && row[phoneKey]) {
                                                                numbers.push(String(row[phoneKey]));
                                                            }
                                                        });

                                                        if (numbers.length > 0) {
                                                            const currentFilter = form.getFieldValue('audience_filter') || {};
                                                            form.setFieldsValue({
                                                                audience_filter: {
                                                                    ...currentFilter,
                                                            ...
                                                            message.success(`Found ${numbers.length} phone numbers!`);
                                                        } else {
                                                            message.error("Could not find any column with phone numbers. Please ensure your column is named 'Phone', 'Mobile', etc.");
                                                        }
                                                    } catch (err) {
                                        ...
                                        >
                                            <Button icon={<FileExcelOutlined />} type="dashed" size="large" style={{ width: '100%' }}>
                                    ...
                                    <>
                                        ...
                                        <div style={{ marginTop: '24px' }}>
                                            ...
                                            <div style={{ background: '#f8f9fa', padding: '20px', borderRadius: '12px', border: '1px solid #eef0f2' }}>
                                                ...
                                                <>
                                                {(() => {
                                                    const bodyMatches = text.match(/\{\{[0-9]+\}\}/g) || [];
                                                    const bodyVars = [...new Set(bodyMatches)].map(v => ({
                                                        type: 'body',
                                                        variable: v,
                                                        varNum: v.replace(/\{\{|\}\}/g, '')
                                                    }));
                                                    ...
                                                    const allVars = [...bodyVars, ...buttonVars];
                                                    ...
                                                    return allVars.map((v, i) => {
                                                        const isButtonVar = v.type === 'button';
                                                        const mappingKey = isButtonVar ? `button_${v.buttonIdx}_${v.varNum}` : `body_${v.varNum}`;
                                                                ...
                                                                <div style={{ flex: 1 }}>
                                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                                                        <div style={{ fontSize: '12px', fontWeight: '500', color: '#595959' }}>
                                                                            {isButtonVar ? `Button URL Variable (${v.buttonText})` : 'Body Text Variable'} : <code style={{ color: isButtonVar ? '#d48806' : '#eb2f96', background: isButtonVar ? '#fff1b8' : '#fff0f6', padding: '2px 4px', borderRadius: '4px' }}>{v.variable}</code>
                                                                        </div>
                                                                        {isButtonVar && <Tag color="warning">Dynamic Link</Tag>}
                                                                    </div>
                                                                    
                                                                    {!isButtonVar ? (
                                                                        <>
                                                                            <Form.Item 
                                                                                name={['variable_mapping', mappingKey]} 
                                                                                rules={[{ required: true, message: 'Mapping is required' }]}
                                                                                style={{ marginBottom: '8px' }}
                                                                            >
                                                                                <Select 
                                                                                    placeholder={`Select source for ${v.variable}`}
                                                                                    style={{ width: '100%' }}
                                                                                    bordered={false}
                                                                                    className="mapping-select"
                                                                                    dropdownStyle={{ borderRadius: '8px' }}
                                                                                >
                                                                                    <Option key="header_lead" value="header_lead" disabled style={{ background: '#f5f5f5', color: '#8c8c8c' }}>-- Lead Data --</Option>
                                                                                    ...
                                                                                    {getFieldValue('target_module') === 'excel' && getFieldValue(['audience_filter', 'excel_data']) && (
                                                                                        <>
                                                                                            <Option key="header_excel" value="header_excel" disabled style={{ background: '#f5f5f5', color: '#8c8c8c' }}>-- Excel Columns --</Option>
                                                                                            {Object.keys(getFieldValue(['audience_filter', 'excel_data'])[0] || {})
                                                                                                .filter(col => !col.startsWith('__'))
                                                                                                .map(col => (
                                                                                                    <Option key={`excel_${col}`} value={`excel_${col}`}>Column: {col}</Option>
                                                                                            ))}
                                                                                        </>
                                                                                    )}
                                                                                    {/* Dynamic Custom Fields */}
                                                                                    {customFields.length > 0 && <Option key="header_custom" value="header_custom" disabled style={{ background: '#f5f5f5', color: '#8c8c8c' }}>-- Custom Fields --</Option>}
                                                                                    {customFields.map(f => (
                                                                            ...
                                                                            <Form.Item 
                                                                                noStyle 
                                                                                shouldUpdate={(prev, curr) => prev.variable_mapping?.[mappingKey] !== curr.variable_mapping?.[mappingKey]}
                                                                            >
                                                                                {({ getFieldValue }) => 
                                                                                    getFieldValue(['variable_mapping', mappingKey]) === 'custom' ? (
                                                                                        <Form.Item 
                                                                                            name={['custom_variables', mappingKey]} 
                                                                                            rules={[{ required: true, message: 'Value is required' }]}
                                                                                            style={{ marginBottom: 0 }}
                                                                                        >
                                                                                            <Input placeholder="Enter static value" style={{ borderRadius: '6px' }} />
                                                                                        </Form.Item>
                                                                                    ) : null
                                                                                }
                                                                            </Form.Item>
                                                                        ...
                                                                        <Form.Item 
                                                                            name={['variable_mapping', mappingKey]} 
                                                                            rules={[{ required: true, message: 'URL value is required' }]}
                                                                            style={{ marginBottom: 0 }}
                                                                        >
                                                                            <Input placeholder="Enter URL value (e.g. www.leadgoes.com or landing-page)" style={{ borderRadius: '6px' }} />
                                                                        </Form.Item>
                                                                    )}
                                                                </div>
                                                            </div>
