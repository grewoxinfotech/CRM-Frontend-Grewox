                    ...
                    <Col span={24}>
                        ...
                        >
                            <Select
                                ...
                                showSearch
                                filterOption={(input, option) =>
                                    option?.data?.username?.toLowerCase().includes(input.toLowerCase())
                                }
                                maxTagCount={2}
                                maxTagTextLength={10}
                                ...
                                tagRender={({ label, value, closable, onClose }) => {
                                    ...
                                    return (
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '2px 6px', borderRadius: 6, background: '#f4f6fa', marginRight: 4 }}>
                                            ...
                                            <span style={{ fontWeight: 500, fontSize: 13 }}>{emp.username}</span>
                                            <span style={{ padding: '1px 8px', borderRadius: 4, fontSize: 11, background: roleStyle.bg, color: roleStyle.color, border: `1px solid ${roleStyle.border}`, fontWeight: 500, textTransform: 'capitalize' }}>{getRoleName(emp.role)}</span>
                                            {closable && (
                                                <span onClick={onClose} style={{ cursor: 'pointer', marginLeft: 4, color: '#bbb' }}>×</span>
                                            )}
                                        </div>
                                    );
                                }}
                                    ...
                                    return {
                                        ...
                                        label: (
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '4px 0' }}>
                                                <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#e6f4ff', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1890ff', fontSize: 15, fontWeight: 500, textTransform: 'uppercase', overflow: 'hidden' }}>
                                                    {emp.avatar ? (
                                                        <img src={emp.avatar} alt={emp.username} style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover' }} />
                                                    ) : (
                                                        emp.username?.charAt(0) || '?'
                                                ...
                                                <span style={{ fontWeight: 500, color: 'rgba(0, 0, 0, 0.85)', fontSize: 14 }}>{emp.username}</span>
                                                <span style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '12px', background: roleStyle.bg, color: roleStyle.color, border: `1px solid ${roleStyle.border}`, fontWeight: 500, textTransform: 'capitalize', marginLeft: 8 }}>{getRoleName(emp.role)}</span>
                                            </div>
                                        ),
                                    };
                                })}
