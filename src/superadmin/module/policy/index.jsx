...
import {
    ...
    Dropdown, Menu, Row, Col, Breadcrumb, Table, Spin, Empty
} from 'antd';
import {
    FiPlus, FiSearch,
...
const Policy = () => {
    ...
    // API hooks
    const { data: policiesData, isLoading: isPoliciesLoading, error: policiesError } = useGetAllPoliciesQuery(undefined, {
    ...
    const handleDeletePolicy = async () => {
        try {
            ...
            if (error?.data?.message?.includes('AccessDenied') || 
    ...
    return (
        <div className="policy-page">
            ...
            <Card className="policy-table-card">
                {viewMode === 'table' ? (
                    ...
                    <Row gutter={[16, 16]} className="policy-cards-grid">
                        {!isPoliciesLoading && filteredPolicies.map(policy => (
            ...
            >
                <p>Are you sure you want to delete <strong>{selectedPolicy?.name}</strong>?</p>
