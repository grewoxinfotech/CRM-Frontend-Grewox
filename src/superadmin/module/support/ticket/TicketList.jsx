...
const TicketList = ({ tickets, onEdit, onDelete, onView, loading, isSupport = false }) => {
    ...
    const getEmployeeName = useCallback((employeeId, record) => {
        if (!employeeId) return 'N/A';
        
        if (isLoadingEmployees) return 'Loading...';
        
        const employee = employeesMap[employeeId];
        if (employee) return employee.name || employee.email || `Employee ${employeeId}`;
        
        return record?.requestorName || `Unknown (ID: ${employeeId})`;
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: 'requestor',
            key: 'requestor',
            render: (requestorId, record) => getEmployeeName(requestorId, record),
            sorter: (a, b) => {
                const nameA = getEmployeeName(a.requestor, a);
                const nameB = getEmployeeName(b.requestor, b);
